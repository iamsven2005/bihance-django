...
from applications.models import JobType, PayType, DurationType
...
class JobCreateInputSerializer(serializers.Serializer): 
    ...
    jobRequirements = serializers.ListField(required=False, child=serializers.CharField(), allow_empty=False)        
    ...
    def validate(self, data): 
        ...
        if data.get("higherSalary") and data.get("salary") and data["higherSalary"] < data["salary"]:
            raise serializers.ValidationError("Higher salary cannot be less than salary.")
        
        if data.get("startAge") and data.get("endAge") and data["endAge"] < data["startAge"]:
            raise serializers.ValidationError("End age must be greater than or equal to start age.")
...
class JobPartialUpdateInputSerializer(serializers.Serializer): 
    ...
    def validate(self, data): 
        ...
        if data.get("startDate") and data.get("endDate") and data["endDate"] < data["startDate"]:
            raise serializers.ValidationError("End date must be after start date.")
