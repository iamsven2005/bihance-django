...
class ReviewsAPITest(TestCase):
    ...
    def test_update_review(self):
        ...
        self.auth_employee()
        data = {"content": "This job application process was so ass.", "rating": "1"}
        response = self.client.patch(
            f"{self.base_url}{application_id}/", data, format="json"
        )
        self.assertEqual(response.status_code, 200)

        self.auth_employer()
        data = {"content": "This applicant was soo fucking rude."}
        response = self.client.patch(
            f"{self.base_url}{application_id}/", data, format="json"
        )
        self.assertEqual(response.status_code, 200)
